...
function ScoreRing({ score, max }: { score: number; max: number }) {
  ...
  return (
    <svg width="80" height="80" viewBox="0 0 80 80">
      ...
      />
      <text x="40" y="41" textAnchor="middle" dominantBaseline="middle" fontSize="15" fontWeight="500" fill="var(--color-text-primary)">
        {score}
      </text>
      <text x="40" y="56" textAnchor="middle" dominantBaseline="middle" fontSize="10" fill="var(--color-text-secondary)">
        / {max}
      </text>
    </svg>
  );
...
function Toast({ message, type, onClose }: { message: string; type: ToastType; onClose: () => void }) {
  ...
  return (
    ...
    >
      <span>{message}</span>
      <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "inherit", fontSize: 16 }}>
        ✕
      </button>
    </div>
  );
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          {rules.map((rule) => {
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 0.8fr", gap: 8, marginBottom: 8 }}>
                    <input
                      value={editForm.atributo}
                      onChange={(e) => setEditForm({ ...editForm, atributo: e.target.value })}
                    ...
                    <input
                      ...
                      onChange={(e) => setEditForm({ ...editForm, puntaje: parseInt(e.target.value) || 0 })}
                      placeholder="Puntaje"
                    />
                  </div>
            ...
            return (
              <div key={rule.id} style={{ border: "1px solid var(--color-border-tertiary)", padding: "0.5rem", borderRadius: "var(--border-radius-md)" }}>
                <div>
                  <strong>{rule.nombre}</strong> {rule.activa ? "✅ Activa" : "❌ Inactiva"}
                </div>
                <div style={{ fontSize: 12, color: "var(--color-text-secondary)" }}>{rule.descripcion || "Sin descripción"}</div>
                <div style={{ fontSize: 11, marginTop: 4 }}>
                  Condición: {cond.atributo} {cond.operador} {cond.valor} → +{rule.condiciones_json.puntaje} pts
                </div>
      ...
      <div style={cardStyle}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 0.8fr", gap: 8, marginBottom: 8 }}>
          <input
            placeholder="Atributo"
            value={newRule.atributo}
            onChange={(e) => setNewRule({ ...newRule, atributo: e.target.value })}
          />
          <input
            placeholder="Operador"
            value={newRule.operador}
            onChange={(e) => setNewRule({ ...newRule, operador: e.target.value })}
          />
          <input
            placeholder="Valor"
            value={newRule.valor}
          ...
          <input
            type="number"
            placeholder="Puntaje"
            value={newRule.puntaje}
            onChange={(e) => setNewRule({ ...newRule, puntaje: parseInt(e.target.value) || 0 })}
          />
        </div>
        <label style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
          <input
            type="checkbox"
            checked={newRule.activa}
            onChange={(e) => setNewRule({ ...newRule, activa: e.target.checked })}
          />{" "}
          Activa
        </label>
        <button
...
function AppContent() {
  ...
  const [initialDataLoaded, setInitialDataLoaded] = useState(false); // Control para evitar 401 iniciales
  ...
  const loadConversations = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // No hacer nada, solo significa que no hay token aún
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const createNewConversation = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/chat`, {
        ...
        body: JSON.stringify({ message: "", title: "Nueva conversación", document_ids: [] }),
      });
      if (res.ok) {
        const data = await res.json();
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.log("📄 Documentos recibidos del backend:", JSON.stringify(docs, null, 2));
      
      const loadedResults = docs.map((doc: any) => {
        // Convertir active_rules al formato esperado por el frontend
        let activeRules = [];
        
        // Si active_rules es un string JSON, parsearlo
        if (typeof doc.active_rules === 'string') {
          try {
            activeRules = JSON.parse(doc.active_rules);
          } catch (e) {
            console.error("Error parsing active_rules:", e);
            activeRules = [];
          }
        } else if (Array.isArray(doc.active_rules)) {
          activeRules = doc.active_rules;
        }
        
        // Formatear las reglas para el frontend
        const formattedRules = activeRules.map((r: any) => ({
          rule: r.nombre || r.rule || "Regla sin nombre",
          points: r.puntaje || r.points || 0,
          active: true
        }));
  ...
  // Cargar datos solo cuando hay token y solo una vez después del login
  useEffect(() => {
    if (token && !initialDataLoaded) {
      loadDocuments();
      loadConversations();
      setInitialDataLoaded(true);
    } else if (!token && initialDataLoaded) {
      // Resetear cuando se cierra sesión
      setResults([]);
      setMessages([INITIAL_MESSAGE]);
      setUploadedDocIds([]);
      setConversations([]);
      ...
      try {
        ...
        if (res.ok) {
          ...
          const assistantMsg: ChatMessage = {
            ...
            role: "assistant",
            text: data.response || "Mensaje recibido. ¿En qué más puedo ayudarte?",
            ts: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
          };
  ...
  // ── Render Análisis (UI completa, sin cambios estructurales) ───────────────
      ...
      <div style={{ display: "flex", gap: "1rem", alignItems: "start" }}>
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
            ...
            <button onClick={() => setShowSidebar(false)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 16 }}>
              ✕
            </button>
          </div>
          <button
            onClick={createNewConversation}
            disabled={isCreatingConv || !token}
            style={{
              width: "100%",
              padding: "6px 12px",
              background: "var(--color-background-info)",
              ...
              border: "none",
              borderRadius: "var(--border-radius-md)",
              cursor: isCreatingConv || !token ? "not-allowed" : "pointer",
              marginBottom: "1rem",
              opacity: isCreatingConv || !token ? 0.5 : 1,
            }}
          >
            {isCreatingConv ? "Creando..." : "+ Nueva conversación"}
          </button>
            ...
            {conversations.map((conv) => (
              ...
              >
                ...
                <button
                  ...
                  style={{ background: "none", border: "none", cursor: "pointer", color: "var(--color-text-danger)", fontSize: 14 }}
                >
                  🗑️
                </button>
              </div>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 360px", gap: "1.5rem", alignItems: "start" }}>
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <i
                    ...
                    style={{ fontSize: 28, color: dragging ? "var(--color-text-info)" : "var(--color-text-secondary)", flexShrink: 0 }}
                  />
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 500 }}>{dragging ? "Suelta los archivos aquí" : "Arrastra uno o varios documentos"}</div>
                    <div style={{ fontSize: 11, color: "var(--color-text-secondary)" }}>PDF o DOCX · Múltiples archivos · Máx. 10 MB c/u</div>
                  </div>
                  <div style={{ marginLeft: "auto" }}>
                    <Badge type="info">Multi-archivo</Badge>
                  </div>
                </div>
              </div>
                  ...
                  <div key={result.id} style={{ animation: "fadeIn 0.3s ease" }}>
                    {result.status === "loading" && (
                      <div style={{ ...card, display: "flex", alignItems: "center", gap: 12 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: 12, fontWeight: 500, marginBottom: 6 }}>{result.fileName}</div>
                    ...
                    {result.status === "error" && (
                      <div style={{ ...card, display: "flex", alignItems: "center", gap: 12 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: 12, fontWeight: 500 }}>{result.fileName}</div>
                          <div style={{ fontSize: 11, color: "var(--color-text-danger)" }}>Error al conectar con el servidor</div>
                        </div>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, marginBottom: "0.75rem" }}>
                          ...
                          >
                            ...
                            <div style={{ fontSize: 18, fontWeight: 500 }}>
                              {result.score}
                              <span style={{ fontSize: 10, color: "var(--color-text-secondary)", marginLeft: 3 }}>/ {result.maxScore}</span>
                            </div>
                          </div>
                          <div
                            style={{
                          ...
                          >
                            ...
                            <div style={{ fontSize: 18, fontWeight: 500 }}>
                              ...
                              <span style={{ fontSize: 10, color: "var(--color-text-secondary)", marginLeft: 3 }}>/ {result.totalRules}</span>
                            </div>
                          </div>
                          <div
                            style={{
                                ...
                                >
                                  ...
                                  <span
                                    style={{
                                      fontSize: 11,
                                      fontWeight: 500,
                                      color: r.active ? "var(--color-text-success)" : "var(--color-text-secondary)",
                                    }}
                                  >
                                    +{r.points} pts
                                  </span>
            ...
            {/* Columna derecha: chat responsivo */}
            <div
              style={{
                ...card,
                display: "flex",
            ...
            >
              ...
              >
                ...
                <div>
                  <div style={{ fontSize: 12, fontWeight: 500 }}>Asistente IA</div>
                  <div style={{ fontSize: 10, color: "var(--color-text-secondary)" }}>Ollama 3B local</div>
                </div>
                <div style={{ marginLeft: "auto" }}>
              ...
              <div style={{ borderTop: "0.5px solid var(--color-border-tertiary)", paddingTop: "0.6rem" }}>
                ...
                >
                  ...
                  <button
                    onClick={sendMessage}
                    disabled={!input.trim() || chatLoading || isCreatingConv}
                    style={{
                      width: 28,
                      height: 28,
                      borderRadius: "var(--border-radius-md)",
                      background: input.trim() && !chatLoading && !isCreatingConv ? "var(--color-background-info)" : "transparent",
                      border: "none",
                      cursor: input.trim() && !chatLoading && !isCreatingConv ? "pointer" : "not-allowed",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                    }}
                  >
                    <i
                      ...
                      style={{
                        fontSize: 14,
                        color: input.trim() && !chatLoading && !isCreatingConv ? "var(--color-text-info)" : "var(--color-text-secondary)",
                      }}
                    />
                  </button>
                </div>
    ...
    return (
      <div style={{ background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-lg)", padding: "1rem" }}>
        <div style={{ fontSize: 12, color: "var(--color-text-secondary)", marginBottom: 12 }}>
          {done.length} documento(s) analizados · ordenados por puntaje real
        </div>
        {done.map((c, i) => {
          ...
          return (
            <div
              ...
              style={{
                background: "var(--color-background-primary)",
                border: i === 0 ? "2px solid var(--color-border-success)" : "0.5px solid var(--color-border-tertiary)",
                borderRadius: "var(--border-radius-lg)",
            ...
            >
              <div
                style={{
                  ...
                  borderRadius: "50%",
                  flexShrink: 0,
                  background: i === 0 ? "var(--color-background-success)" : "var(--color-background-secondary)",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                  fontSize: 12,
                  fontWeight: 500,
                  color: i === 0 ? "var(--color-text-success)" : "var(--color-text-secondary)",
                }}
              >
                #{i + 1}
              </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                {i === 0 && <Badge type="success">Mejor candidato</Badge>}
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 20, fontWeight: 500, color }}>{c.score}</div>
                  <div style={{ fontSize: 10, color: "var(--color-text-secondary)" }}>/ {c.maxScore} pts</div>
                </div>
              </div>
            </div>
          );
      ...
      <>
        ...
        <div style={card}>
          ...
          {roadmap.map((item) => (
            <div
              key={item.label}
              style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 0", borderBottom: "0.5px solid var(--color-border-tertiary)" }}
            >
  ...
  const renderFaq = () => {
    const faqs = [
      {
        q: "¿A quién reporto si el sistema falla?",
        a: "Escríbenos a 📧 alvaradoluis2002@gmail.com indicando el archivo, error y captura. Respuesta en <24h.",
      },
      {
        q: "Subí el documento pero nunca cambia a 'Completado'",
        a: "Espera 30 segundos y recarga. Si persiste, el archivo puede estar corrupto o protegido.",
      },
  ...
  // Pantalla de login (sin botón de Google para evitar errores de origen)
  if (showLogin) {
    return (
      <div
        style={{
          ...
          {!isRegister ? (
            <>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
                ...
                <a href="#" style={{ fontSize: 12, color: "var(--color-text-info)", textDecoration: "none" }}>
            ...
            <>
              ...
              <input
                ...
                value={registerData.empresa_nombre}
                onChange={(e) => setRegisterData({ ...registerData, empresa_nombre: e.target.value })}
                style={{
                  width: "100%",
                  marginBottom: "0.75rem",
                  padding: "10px",
              ...
              <input
                type="text"
                placeholder="Empresa NIT"
                value={registerData.empresa_nit}
                onChange={(e) => setRegisterData({ ...registerData, empresa_nit: e.target.value })}
                style={{
                  width: "100%",
                  marginBottom: "1rem",
                  padding: "10px",
